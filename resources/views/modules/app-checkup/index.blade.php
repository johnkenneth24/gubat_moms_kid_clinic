                        ...
                        <div class="card-patient mb-2">
                            <div class="">
                                ...
                                <div class="d-flex">
                                    ...
                                    <p class="ms-2">STATUS: <span class="text-uppercase">{{ $book_app->status }}</span>
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex">
                                ...
                                <p class="ms-2">TIME:
                                    <span>{{ date('h:i A', strtotime($book_app->time_appointment)) }}</span></p>
                            </div>
                            <div class="d-flex mt-2">
                                @role('staff')
                                    <a href="{{ route('app-checkup.view', $book_app->id) }}"
                                        class="btn btn-sm btn-success">VIEW</a>
                                    @if (!$book_app->bookAppConsult)
                                        <button type="button" class="btn btn-primary btn-sm mx-1" data-bs-toggle="modal"
                                            data-bs-target="#preconsult{{ $book_app->id }}">
                                            PRE CONSULT
                                        </button>
                                        <a href="{{ route('app-checkup.noshow', $book_app->id) }}"
                                            class="btn btn-sm btn-danger">DID NOT ATTEND</a>
                                    @endrole
                                @endif
                                @role('pediatrician')
                                    <a href="{{ route('app-checkup.book-view-med', $book_app->id) }}"
                                        class="btn btn-sm btn-primary mx-1">MEDICAL HISTORY</a>
                                    @if ($book_app->bookAppConsult)
                                        <a href="{{ route('app-checkup.consult', $book_app->id) }}"
                                            class="btn btn-sm btn-info">CONSULT</a>
                                    @endif
                                @endrole
                            </div>
                        </div>

                        <div class="modal fade" id="preconsult{{ $book_app->id }}" tabindex="-1" data-bs-backdrop="static"
                            aria-labelledby="exampleModalLabel" aria-hidden="true" style="width: 100%;">
                            <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h6 class="modal-title text-uppercase" id="exampleModalLabel">PRE CONSULT |
                                            {{ $book_app->user->full_name }}</h6>
                                        </button>
                                    </div>
                                    <form action="{{ route('app-checkup.pre-consult', [$book_app]) }}" method="post">
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                ...
                                                <input step="0.01" type="number" name="weight"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input step="0.01" type="number" name="height"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" placeholder="0/0" name="blood_pressure"
                                                    pattern="^\d+\/\d+$" value="{{ old('blood_pressure') }}"
                                                    class="form-control">
        ...
        <div class="col-lg-6">
            <div class="card p-2 card-app">
                <div class="card-header d-flex align-items-center justify-content-between">
                    ...
                    <div class="card-tool">
                        @role('staff')
                            <a href="{{ route('walkin-appointment.create') }}" class="btn btn-primary btn-sm"
                                style="color: #ffff;">ADD WALK-IN</a>
                        @endrole
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="card-patient mb-2">
                            <div class="">
                                ...
                                <div class="d-flex">
                                    ...
                                    <p class="ms-2">STATUS: <span class="text-uppercase">{{ $walkin->status }}</span>
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex">
                                ...
                                <p class="ms-2">TIME:
                                    <span>{{ date('h:i A', strtotime($walkin->time_consultation)) }}</span></p>
                            </div>
                            <div class=" mt-2">
                                @role('staff')
                                    <a href="{{ route('walkin-appointment.view', $walkin->id) }}"
                                        class="btn btn-sm btn-success">VIEW</a>
                                    <a href="{{ route('walkin-appointment.edit', $walkin->id) }}"
                                        class="btn btn-sm btn-primary mx-1">UPDATE</a>
                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#walkInDelete{{ $walkin->id }}">
                                        DELETE
                                    </button>
                                @endrole
                                @role('pediatrician')
                                    <a href="{{ route('app-checkup.view-med', $walkin->id) }}"
                                        class="btn btn-sm btn-primary me-1">MEDICAL HISTORY</a>
                                    <a href="{{ route('walkin-appointment.consult', $walkin->id) }}"
                                        class="btn btn-sm btn-info">CONSULT</a>
                                @endrole
                            </div>
                        </div>

                        <div class="modal fade" id="walkInDelete{{ $walkin->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <h5 class="text-center mb-0" style="line-height: unset;">Are you sure you want to
                                            delete <br> <span
                                                class="text-primary">{{ $walkin->walkInPatient->full_name }}</span>
                                            appointment?</h5>
                                    </div>
